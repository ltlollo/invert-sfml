...
void Inverter::run() { invert_points(); new_png.write(oname); }

void Inverter::operator()() { run(); }

void Inverter::set_center(const Coord p)
{ center = p; gtmodsq_maps_outside = find_max_radiussq(); }

void Inverter::set_radius(const int d)
{ radius = d; rsq = d*d; gtmodsq_maps_outside = find_max_radiussq(); }

Coord Inverter::get_center() const noexcept { return center; }

int Inverter::get_radius() const noexcept { return radius; }
